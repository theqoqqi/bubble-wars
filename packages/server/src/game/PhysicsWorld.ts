...
export class PhysicsWorld {
  ...
  private createWalls(): void {
    ...
    const wallOpts: Matter.IBodyDefinition = {
      ...
      collisionFilter: {
        ...
        mask: COLLISION_CATEGORIES.TANK | COLLISION_CATEGORIES.PROJECTILE | COLLISION_CATEGORIES.OBSTACLE,
      },
      label: 'wall',
    };
  ...
  public step(deltaMs: number): void {
    ...
    for (const o of this.obstacles) {
      ...
      if (px > width - margin) fx -= ((px - (width - margin)) / margin) * 0.0025 * o.body.mass;
      if (py < margin) fy += ((margin - py) / margin) * 0.0025 * o.body.mass;
      if (py > height - margin) fy -= ((py - (height - margin)) / margin) * 0.0025 * o.body.mass;
